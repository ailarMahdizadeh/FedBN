...
class MAHGCNNET(nn.Module):
    def __init__(self,ROInum,layer, num_class=2):
        ...
        self.MAHGCN = MAHGCN.MAHGCN(nn.ReLU(),0.2,self.layer)
...
class GCN_gpool(nn.Module):
    def __init__(self,ROInum,num_class=2):
        ...
        self.gcn = MAHGCN.GraphUnet([1/2],ROInum, 1, 1, nn.ReLU(),0.2) 

        self.bn1 = torch.nn.BatchNorm1d(592)
        self.fl1 = nn.Linear(592,64)
        self.bn2 = torch.nn.BatchNorm1d(64)
        self.fl2 = nn.Linear(64,num_class)
    ...
    def forward(self, g):
        ...
        fea = fea.cuda()
        g = g.cuda()
        out = torch.zeros(batch_size, 592)
...
class GCN_diffpool(nn.Module):
    def __init__(self,ROInum,num_class=2):
        ...
        self.gcn = MAHGCN.GraphDiif([1/2],ROInum, 1, 1, nn.ReLU(),0.2) 
...
class GCN_SAG(nn.Module):
    def __init__(self,ROInum,num_class=2):
        ...
        self.gcn = MAHGCN.GraphSAG([1/2],ROInum, 1, 1, nn.ReLU(),0.2) 
